...
[자율 조립 규칙](PR_WORKFLOW.md)에서 단위 선택·실제 커밋·역할 PR·AI 교차 검토·후보 통합·전체 검사 후 최종 병합을 정한다. 3번 Kiro가 통합하고 1번 Kiro가 최종 후보를 검토한다. 공식 저장소/base는 팀 설정으로 지정하며 미완성 역할 PR을 제출 브랜치에 부분 병합하지 않는다.
