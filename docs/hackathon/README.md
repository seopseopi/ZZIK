...
2. **2단계:** API 9개 라우터·화면 9개 컴포넌트 파일 분리. 독립 실행기의 API/worker 재시작 검사와 GitHub의 Compose 전체 실행·컨테이너 재시작 검사 추가.
3. **이번 3단계 — 서비스 검증:** [교육 계정 검증](../AWS_EDU_VALIDATION.md). 임시 AWS 인증과 앱 분석 함수의 합성 사진 검증 완료(실제 호출 6회, 얼굴 구분·바다 태그 확인). 자동 실행기 및 [EC2 인계용 컨테이너](../AWS_VALIDATION_CONTAINER.md) 준비. EC2 역할·S3·API/worker/DB 종단 연결·AWS 배포와 실사진·여러 기기·AWS 복원·처리량 검증은 남아 있음. 로컬 DB·사진은 [백업본만으로 복원하는 리허설](../BACKUP_RESTORE.md)을 추가해 검증.
4. **응답 계약:** JSON 응답 모델·파일 MIME 선언과 OpenAPI 기반 프론트 타입 생성 연결. [계약 갱신·검사](../../contracts/README.md).
5. **대회 조립판:** 검증된 기준본을 보존하고 별도 starter를 제작. 팀원이 작업별로 다시 구현·통합하여 같은 검사를 통과하는지 리허설.

현재는 **로컬 검증 기준본과 별도의 역할별 starter 생성 도구**를 준비한 단계다. [starter 가이드](../../starter/README.md)에 5개 역할·M01~M17·30개 구현 단위와 검사 절차가 있다. AWS 운영 완료본은 아니며, GitHub Pages는 프론트 체험판이다.
...
계약 검사에서는 작업 의존성·참조 파일·프롬프트 일치·실제 API 구현 파일 대응·중복 경로·API 경로/요청·응답 스키마·Python 매개변수 이름을 확인한다. 실제 응답의 의미와 AWS 연결 품질은 통합 테스트와 실환경 검증이 별도로 필요하다.
