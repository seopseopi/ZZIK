...
3. **이번 3단계 — 서비스 검증:** [교육 계정 검증](../AWS_EDU_VALIDATION.md). 임시 AWS 인증과 앱 분석 함수의 합성 사진 검증 완료(실제 호출 6회, 얼굴 구분·바다 태그 확인). 자동 실행기 및 [EC2 인계용 컨테이너](../AWS_VALIDATION_CONTAINER.md) 준비. EC2 역할·S3·API/worker/DB 종단 연결·AWS 배포와 실사진·여러 기기·복원·처리량 검증은 남아 있음.
