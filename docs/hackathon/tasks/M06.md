...
내 사진·단체·얼굴 미검출·인물 조합을 서버 필터로 제공한다.

## 출발 상태

구현·로컬 검증됨; 실제 분석 결과와 연결 검증 필요. 이 상태는 계획 작성 시점의 기준이며 이번 작업의 완료 증거가 아니다. 기존 구현과 테스트를 읽고 부족한 부분만 변경한다.
...
- [ ] mine와 혼자/함께 조건 동시 적용
- [ ] 여러 인물 AND/OR 및 페이지네이션의 total 일치
- [ ] 모바일/PC 빈 결과·로딩·실패와 이미지 경로 정상
