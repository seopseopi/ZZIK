...
| 등록 인물 분석 | 분석 함수 합성 세트 실검증 | 실제 Rekognition 호출로 등록 1명·미등록 1명/얼굴 없음 확인. API·worker·DB 종단 연결과 실사진 정확도는 미검증 |
...
| 장면·구체적 장소 | 장면 함수 합성 세트 실검증 | 실제 DetectLabels로 합성 풍경의 바다 태그 확인. 실사진 품질·역지오코딩 미검증, 외부 서비스 미설정 시 GPS 좌표 유지 |
...
- 교육용 AWS 콘솔 확인에 이어 앱 분석 함수의 합성 사진 실검증을 통과했다. 앱에서 사용할 EC2·S3는 생성하지 않았고 역할 기반 저장·분석, 실사진 정확도·자동 그룹의 종단 검증은 미실시다. 외부 연결 없이 로컬 파일/DB/보정/수동 인물/승인은 동작한다.
...
### M03·M05·M13 — 앱 분석 함수의 실제 AWS 검증

- `--analysis-only`를 추가했다. 명시적인 분석 전용 실행은 버킷 없이 수행하며, 계정 일치를 검사하고 성공을 `analysis_passed`로 구분한다. 기본 S3 검증과 호출 상한은 유지한다.
- 코드 커밋 `7d9e4e9`에서 합성 기준 사진 1장·평가 사진 2장으로 실제 분석 함수를 실행했다. 총 6회 호출했고 얼굴 2개/등록 1명/미등록 1명, 풍경 얼굴 0개·`바다` 태그를 확인했다. [상세 결과와 한계](AI_VALIDATION.md).
- 관련 로컬 테스트 63개, 하네스 검사 통과. CI는 분석 전용 컨테이너 plan을 네트워크 없이 검사하며 실제 AWS를 호출하지 않는다.
- 신규 EC2·S3·컬렉션은 생성하지 않았다. S3 저장·EC2 역할·API/worker/DB 종단 연결·자동 인물 그룹·실사진 정확도·대량 성능은 미완료다. 서비스 `.env`는 local/fixture를 유지한다.


### M01 — 로컬 DB·사진 백업과 복원

- `backup_local.py`: 쓰기가 멈춘 로컬 PostgreSQL과 사진 폴더를 해시 manifest와 함께 백업하고, 검증된 백업을 새 임시 DB/사진 폴더에 복원한다. 기존 데이터 덮어쓰기는 제공하지 않는다.
- `verify_backup_restore.py`: API/worker로 보정·승인·최종본을 만든 뒤 백업, **소스 테스트 DB·사진 삭제**, 새 DB·사진 복원, 기존 세션과 원본/보정본 SHA-256·승인·최종본 확인. 복원 후 새 업로드·worker 분석도 통과했다.
...
[시나리오·담당·증거 양식](THREE_TIER_REHEARSAL.md)을 준비했다. Web/App/RDS 실분리 배포, App 중단 중 정적 화면과 오류 안내, 별도 경로 RDS/S3 보존 확인, AWS 복구 후 기능 검증은 모두 미실행이다. 기존 로컬 재시작·백업 복구 통과와 구분한다.
