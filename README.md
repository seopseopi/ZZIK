...
[해커톤 조립 가이드](docs/hackathon/README.md)에 인프라·프론트·백엔드·AI·보정의 **5개 역할 프롬프트와 M01~M17 작업 프롬프트**를 정리했습니다. 역할과 이번 M 작업을 함께 읽고 기능별 PR로 통합합니다. API 라우터와 화면 컴포넌트를 기능별로 분리했습니다. 계약 검사·실제 서버 E2E·Compose 전체 실행·재시작 검사를 CI에서 실행하며, AWS 검증과 별도 starter 제작은 후속 단계입니다.
